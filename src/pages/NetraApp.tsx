...
import { AppState, DetectionResult, DetectedObject } from '../types/netraTypes.ts';
import { identifyObject, detectObjectsLive, chatWithScene, describeLocation } from '../services/netraGeminiService.ts';
import { NetraInfoCard } from '../components/NetraInfoCard.tsx';
...
export const NetraApp: React.FC = () => {
  ...
  return (
    <div className="relative w-full h-full min-h-[500px] aspect-video bg-black overflow-hidden select-none font-sans text-cyan-50 rounded-lg border border-cyan-500/20">
      ...
      <div className="absolute inset-0 pointer-events-none z-10">
         ...
         {/* Corner Brackets */}
         <CornerBracket className="absolute top-4 left-4 text-cyan-500 opacity-80" />
         <CornerBracket className="absolute top-4 right-4 rotate-90 text-cyan-500 opacity-80" />
         <CornerBracket className="absolute bottom-4 left-4 -rotate-90 text-cyan-500 opacity-80" />
         <CornerBracket className="absolute bottom-4 right-4 rotate-180 text-cyan-500 opacity-80" />
      ...
      {result && (
        ...
        >
          <NetraInfoCard result={result} onDismiss={handleStop} />
        </div>
      )}

      {/* --- STATUS OVERLAYS --- */}
      {recognizedText && (
        <div className="absolute top-24 left-1/2 -translate-x-1/2 z-50 pointer-events-none w-full flex justify-center px-4">
            <div className="bg-black/80 border-l border-r border-cyan-500 px-8 py-3 backdrop-blur-md clip-tech-border max-w-full shadow-lg">
                <span className="text-cyan-400 font-mono text-lg uppercase tracking-widest text-center block truncate">
                     "{recognizedText}"
                </span>
            </div>
        </div>
      ...
      {agentMessage && (
        <div className="absolute bottom-32 left-1/2 -translate-x-1/2 z-50 pointer-events-none w-full max-w-lg flex justify-center px-4">
      ...
      {/* --- TOP HEADER --- */}
      <div className="absolute top-0 left-0 w-full p-4 flex justify-between items-start z-30 pointer-events-none bg-gradient-to-b from-black/60 to-transparent">
        ...
        <div className="font-mono text-xs text-cyan-600 text-right pointer-events-auto flex flex-col items-end">
            ...
            {locationName && (
                <div className="text-cyan-400 font-bold uppercase tracking-wider mb-1 flex items-center gap-1">
              ...
              <div className="bg-slate-900 border border-cyan-500/50 p-6 w-full max-w-sm clip-tech-border shadow-[0_0_30px_rgba(6,182,212,0.2)]">
                  ...
                  <div className="space-y-6">
                      <div className="space-y-2">
                          <div className="flex justify-between text-xs font-mono text-cyan-50">
      ...
      {/* --- BOTTOM COMMAND DECK --- */}
      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md z-40 pointer-events-auto p-4 pb-8 bg-gradient-to-t from-black/80 to-transparent">
        <div className="bg-slate-900/80 backdrop-blur-xl border border-cyan-500/20 clip-tech-border p-2 shadow-2xl">
            <div className="grid grid-cols-3 gap-2">
                <button 
                  onClick={() => manualScan()}
                  className="relative h-12 flex flex-col items-center justify-center border transition-all duration-300 clip-tech-corner-tl bg-transparent border-cyan-800/50 text-cyan-400 hover:bg-cyan-900/30 active:scale-95"
                >
                ...
                <button 
                  ...
                  className={`relative h-12 flex flex-col items-center justify-center border transition-all duration-300 active:scale-95 ${isAutoScanEnabled ? 'bg-emerald-500/10 border-emerald-400 text-emerald-300' : 'bg-transparent border-cyan-800/50 text-cyan-400'}`}
                >
                ...
                <button 
                  onClick={() => handleStop()}
                  className="relative h-12 flex flex-col items-center justify-center border transition-all duration-300 clip-tech-corner-br bg-transparent border-cyan-800/50 text-cyan-400 hover:text-red-400 hover:border-red-950 active:scale-95"
                >
